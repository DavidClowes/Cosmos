...
namespace Cozmos
{
	static GLenum ShaderDataTypeToOpenGLBaseType(ShaderDataType type)
	{
		...
		COZ_CORE_ASSERT(false, "Unknown ShaderDataType!");
		return 0;
	}
	...
	{
		COZ_CORE_ASSERT(vertexBuffer->GetLayout().GetElements().size(), "Vertex Buffre has no layout");
		...
		for (const auto& element : layout)
		{
			...
			glVertexAttribPointer(index,                        // Index
				element.GetComponentCount(),                    // Count
